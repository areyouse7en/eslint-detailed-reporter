...
const styles = _.template(
  ...
  pageTemplate = _.template(
    fs.readFileSync(path.join(__dirname, 'templates/main-page.html'), 'utf-8')
  ),
  resultTemplate = _.template(
    fs.readFileSync(
      path.join(__dirname, 'templates/details/result.html'),
      'utf-8'
    )
  ),
  resultDetailsTemplate = _.template(
    fs.readFileSync(
      path.join(__dirname, 'templates/details/details.html'),
      'utf-8'
    )
  ),
  resultSummaryTemplate = _.template(
    fs.readFileSync(
      path.join(__dirname, 'templates/details/summary.html'),
      'utf-8'
    )
  ),
  codeWrapperTemplate = _.template(
    fs.readFileSync(
      path.join(__dirname, 'templates/details/code/code-wrapper.html'),
      'utf-8'
    )
  ),
  codeTemplate = _.template(
    fs.readFileSync(
      path.join(__dirname, 'templates/details/code/code.html'),
      'utf-8'
    )
  ),
  issueTemplate = _.template(
    fs.readFileSync(
      path.join(__dirname, 'templates/details/code/issue.html'),
      'utf-8'
    )
  ),
  summaryDetailsTemplate = _.template(
    fs.readFileSync(
      path.join(__dirname, 'templates/summary/summary-details.html'),
      'utf-8'
    )
  ),
  rulesTemplate = _.template(
    fs.readFileSync(
      path.join(__dirname, 'templates/summary/rules.html'),
      'utf-8'
    )
  ),
  mostProblemsTemplate = _.template(
    fs.readFileSync(
      path.join(__dirname, 'templates/summary/most-problems.html'),
      'utf-8'
    )
  ),
  filesTemplate = _.template(
    fs.readFileSync(
      path.join(__dirname, 'templates/summary/files.html'),
      'utf-8'
    )
  )
...
function getRuleLink(ruleId) {
  ...
  } else if (_.startsWith(ruleId, '@zstack')) {
    ruleId = ruleId.replace('@zstack/', '')
    ruleLink = `http://gitlab.zstack.io/zstackio/zstack-ui-eslint-plugin/blob/master/docs/rules/${ruleId}.md`
  }
  return ruleLink
}
...
function getProblems(messages) {
  if (!_.isEmpty(messages)) {
    return _.map(messages, 'ruleId')
  }
  return []
}
...
function renderResults(results, currDir) {
  return _.map(results, function (result, index) {
    let template = resultTemplate({
      ...
      problems: getProblems(result.messages)
    })
...
function renderRules(rules) {
  return _(rules)
    .groupBy('ruleId')
    .map(function (ruleMessages, ruleId) {
      return {
        ruleId,
...
module.exports.generateTemplate = function generateTemplate(
  results,
  isMultiOn
) {
